...
async function downloadPDF() {
  ...
  try {
    ...
    // --- STRONGER Helper function for safe text ---
    const cleanText = (text) => {
      if (!text) return '';
      // Convert to string
      let cleaned = String(text);
      
      // Remove all emojis, special symbols, and non-printable characters
      cleaned = cleaned.replace(/[\u{1F000}-\u{1FFFF}]/gu, ''); // Remove emojis
      cleaned = cleaned.replace(/[\u{2000}-\u{2BFF}]/gu, ''); // Remove symbols
      cleaned = cleaned.replace(/[^\x20-\x7E]/g, ''); // Remove all non-ASCII printable chars
    ...
    doc.text('CompFund', 15, 20);
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text('Computer Fundamentals Learning Platform', 15, 28);

    // --- Grade Circle ---
    const gradeColors = { 
      'A+': '#10B981', 'A': '#3B5BDB', 'B': '#0EA5E9', 
      'C': '#F59E0B', 'D': '#F97316', 'F': '#F43F5E' 
    };
    const [r, g, b] = hexToRgb(gradeColors[grade.letter] || '#3B5BDB');
    doc.setFillColor(r, g, b);
    doc.circle(W - 25, 22, 14, 'F');
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    doc.setTextColor(255, 255, 255);
    doc.text(grade.letter, W - 25, 26.5, { align: 'center' });
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.text('QUIZ RESULT REPORT', W / 2, y, { align: 'center' });
    y += 8;
    
    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(100, 100, 100);
    const moduleTitle = cleanText(mod.title);
    ...
    // --- STUDENT INFO ---
    const infoData = [
      ['Student Name:', cleanText(state.studentName || 'N/A')],
      ['Module:', moduleTitle],
      ['Date:', date],
      ['Time Taken:', formatTime(state.elapsed || 0)]
    ];
    ...
    const colLabels = ['Score', 'Percentage', 'Grade', 'Status'];
    const colValues = [`${correct}/${total}`, `${pct}%`, grade.letter, pct >= 40 ? 'PASS' : 'FAIL'];
    const colColors = [
      [59, 91, 219],
      [59, 91, 219],
      [r, g, b],
      pct >= 40 ? [16, 185, 129] : [244, 63, 94]
    ];

    colLabels.forEach((label, i) => {
      const x = boxX + 5 + (i * colWidth);
      
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7);
      doc.setTextColor(120, 120, 120);
      doc.text(label, x, boxY + 12);
      
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(13);
      doc.setTextColor(colColors[i][0], colColors[i][1], colColors[i][2]);
      doc.text(String(colValues[i]), x, boxY + 27);
    });
    ...
    doc.setFontSize(9);
    doc.setTextColor(80, 80, 80);
    const msg = `Don't give up, ${cleanText(state.studentName || 'Student')}! ${pct}% — Computer Fundamentals can be challenging at first. Go again!`;
    const msgLines = doc.splitTextToSize(msg, W - 30);
    ...
    // --- ANSWER SUMMARY (COMPLETELY REWRITTEN) ---
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(11);
    doc.setTextColor(30, 30, 30);
    doc.text('Answer Summary', 15, y);
    y += 10;
    ...
    state.questions.forEach((q, i) => {
      ...
      let questionText = String(q.q || '');
      
      // Remove all special characters, emojis, and symbols
      questionText = questionText.replace(/[^a-zA-Z0-9\s\-.,?!:;'"()]/g, '');
      ...
      // Clean options
      const cleanOpts = q.opts.map(opt => {
        if (!opt) return 'N/A';
        return String(opt).replace(/[^a-zA-Z0-9\s\-.,]/g, '').trim();
      });
      ...
      if (isRight) {
        ...
        statusSymbol = 'Skip';
        statusColor = [200, 200, 200];
      } else {
        statusSymbol = 'Wrong';
        statusColor = [244, 63, 94];
      }
      
      // Draw question with status
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(9);
      doc.setTextColor(statusColor[0], statusColor[1], statusColor[2]);
      
      const qDisplay = `${statusSymbol} Q${i+1}. ${questionText}`;
      const wrappedText = doc.splitTextToSize(qDisplay, W - 30);
      ...
      if (!skipped && chosen !== undefined) {
        // Show user's answer
        doc.setTextColor(80, 80, 80);
        const userAns = cleanOpts[chosen] || 'N/A';
        doc.text(`   Your answer: ${userAns}`, 15, y);
        y += 5;
      } else {
        // Show skipped
        doc.setTextColor(150, 150, 150);
        doc.text('   (Skipped)', 15, y);
        y += 5;
      }

      // Show correct answer
      doc.setTextColor(16, 185, 129);
      const correctAns = cleanOpts[q.ans] || 'N/A';
      doc.text(`   Correct: ${correctAns}`, 15, y);
      y += 5;
    ...
    for (let p = 1; p <= pages; p++) {
      ...
      doc.setFontSize(7);
      doc.setTextColor(150, 150, 150);
      doc.text('CompFund — Computer Fundamentals Learning Platform', W / 2, 290, { align: 'center' });
      doc.text(`Page ${p} of ${pages}`, W - 15, 290, { align: 'right' });
    }

    // Save PDF
    const fileName = `CompFund_Result_${cleanText(state.studentName || 'Student').replace(/\s+/g, '_')}_${state.moduleId}.pdf`;
    doc.save(fileName);
